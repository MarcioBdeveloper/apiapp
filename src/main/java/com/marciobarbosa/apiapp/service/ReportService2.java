...
public class ReportService2 {
    ...
    private static final String TEMPLETE = "\\fluxo-caixa-detalhado-gerencial-dinamico.jrxml";
	...
	private void columnCard() throws JRException{
		...
		for(ColunmAgrupamentoRelDTO dto : this.dadosRel) {
			...
			this.verificaTamnhoDaPagina(xUltimoAdd, Y_DEFAULT_ELEMENT_COLUMN1);
	...
	private void montarDetail(List<ColunmAgrupamentoRelDTO> dto) {
		...
		for(ColunmAgrupamentoRelDTO dados : dto) {
			montarReceitaLiqNvinculado(dados);
			montarReceitaLiqVinculado(dados);
		}
	...
	private void montarReceitaLiqVinculado(ColunmAgrupamentoRelDTO dados) {
		//Receita Liquida inicial - Vinculada
		ReceitaLiquidaRelDTO dadosReceitaVinculada = dados.getReceitasLiqInicialVinculada();
		this.pageHeader.addElement(montarRetangulo(this.yUltimoAdd, X_INIT_DETAIL, colorBackGround));
		this.pageHeader.addElement(montarColunaDescricao(X_INIT_DETAIL, this.yUltimoAdd, dadosReceitaVinculada.getNome()));
		this.verificaTamnhoDaPagina(X_INIT_DETAIL, this.yUltimoAdd);
		this.yUltimoAdd += 15;
		...
		this.pageHeader.addElement(montarRetangulo(this.yUltimoAdd, X_INIT_DETAIL, colorBackGround));
		this.pageHeader.addElement(montarColunaDescricao(X_INIT_DETAIL, this.yUltimoAdd, recMunicipioVinc.getNome()));
		this.verificaTamnhoDaPagina(X_INIT_DETAIL, this.yUltimoAdd);
		this.yUltimoAdd += 15;
				
		for(GrupoRelDTO grupoVinc : recMunicipioVinc.getGrupos()) {
					
			JRDesignStaticText elementoGrupo = montarColunaDescricaoGrupo(X_INIT_DETAIL, this.yUltimoAdd, grupoVinc.getNome(), LEFT_IDENT_GRUPO);
			...
			for(SubGrupoRelDTO subGrupoVinc : grupoVinc.getSubgrupos()) {
				...
				JRDesignStaticText elementoSub = montarColunaDescricaoGrupo(X_INIT_DETAIL, this.yUltimoAdd, subGrupoVinc.getNome(), LEFT_IDENT_SUB_GRUPO);
		...
		this.pageHeader.addElement(montarRetangulo(this.yUltimoAdd, X_INIT_DETAIL, colorBackGround));
		this.pageHeader.addElement(montarColunaDescricao(X_INIT_DETAIL, this.yUltimoAdd, despMunicipioVinc.getNome()));
		this.verificaTamnhoDaPagina(X_INIT_DETAIL, this.yUltimoAdd);
		this.yUltimoAdd += 15;
		...
		this.verificaTamnhoDaPagina(X_INIT_DETAIL, yUltimoAdd);
		yUltimoAdd += 15;
	}

	private void montarReceitaLiqNvinculado(ColunmAgrupamentoRelDTO dados) {
	...
	private List<ColunmAgrupamentoRelDTO> montarDados() {
		...
		SubGrupoRelDTO subGrupo3 = new SubGrupoRelDTO();
		subGrupo2.setNome("CONTAS PUBLICAS (ÁGUA/FONTE/ENERGIA)");
		subGrupo2.setValores(Arrays.asList(BigDecimal.ONE));
		
		SubGrupoRelDTO subGrupo4 = new SubGrupoRelDTO();
		subGrupo2.setNome("CONTAS PUBLICAS (ÁGUA/FONTE/ENERGIA)");
		subGrupo2.setValores(Arrays.asList(BigDecimal.ONE));
		
		SubGrupoRelDTO subGrupo5 = new SubGrupoRelDTO();
		subGrupo2.setNome("CONTAS PUBLICAS (ÁGUA/FONTE/ENERGIA)");
		subGrupo2.setValores(Arrays.asList(BigDecimal.ONE));
		
		grupo.setSubgrupos(Arrays.asList(subGrupo, subGrupo2, subGrupo3, subGrupo4, subGrupo5));
		grupo.setTotalizadores(Arrays.asList(new BigDecimal(3)));
		grupo2.setSubgrupos(Arrays.asList(subGrupo,subGrupo3, subGrupo5));
